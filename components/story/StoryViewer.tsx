...
export default function StoryViewer({ apps, creators, initialAppId }: StoryViewerProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black z-50 flex flex-col max-w-md mx-auto">
      ...
      >
        <p className="text-[9px] uppercase tracking-widest opacity-50 mb-2">{t('card.the_problem')}</p>
        ...
        <p className="text-[9px] uppercase tracking-widest opacity-50 mb-1">{t('card.the_solution')}</p>
      ...
      <div className="flex gap-2 px-3 pt-3">
        ...
        <button className="flex-1 bg-white/10 border border-white/20 text-white rounded-xl py-2 text-xs font-semibold">
          {t('viewer.save_creator')}
        </button>
      </div>
      <p className="text-center text-white/20 text-[9px] mt-2">{t('viewer.hint')}</p>
