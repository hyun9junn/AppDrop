...
function Results() {
  ...
  const suggestedCollection = localizeCollection(collections[0])
    ...
    <div className="pb-20 bg-gray-50 min-h-screen">
      ...
      <div className="p-4 flex flex-col gap-3">
        ...
        <div className="border border-indigo-100 rounded-2xl p-3 bg-indigo-50 flex items-center gap-3">
          <span className="text-2xl">{collections[0].emoji}</span>
          <div className="flex-1">
            <p className="text-xs font-bold text-indigo-700">{suggestedCollection.title}</p>
            <p className="text-[10px] text-indigo-400">{suggestedCollection.appIds.length} {t('collections.apps')}</p>
          </div>
